...
### Kubernetes & Ecosystem Integration (v0.37.x – v0.39.x)
...
| v0.39.0 | DuckLake v1.0 Spec Compliance: upgrade `DuckLakeSink` from the 8-table custom catalog to the real 28-table DuckLake v1.0 spec — catalog-wide snapshots with `next_catalog_id`/`next_file_id`, snapshot-carried ID counter allocation (removing all `nextval()` snapshot-ID calls), the complete table set required for DuckDB `ATTACH`, end-to-end DuckDB queryability integration tests, migration helper for existing deployments, complete `DuckLakeSource` read-path, and atomic exactly-once validation | ✅ Released | Large | [plans/ecosystem/ducklake.md](../../plans/ecosystem/ducklake.md) · [plans/ecosystem/rocklake.md](../../plans/ecosystem/rocklake.md) |
...
#### v0.39.0 — DuckLake v1.0 Spec Compliance (detail)

The `DuckLakeSink` introduced in v0.20.0 writes a functional but non-standard catalog. It uses a custom 8-table schema with **per-table** `ducklake_snapshot` rows and allocates all IDs via `nextval()` sequences. The real DuckLake v1.0 specification uses **catalog-wide** snapshots, carries the `next_catalog_id` and `next_file_id` counters inside each snapshot row, and defines 28 tables. As a result, DuckDB cannot currently `ATTACH` to a pg-tide-managed catalog and query it directly — the entire value proposition of the DuckLake integration (DuckDB queryability, time-travel, filter pushdown) depends on closing this gap.

This release brings `DuckLakeSink` into full compliance with the DuckLake v1.0 specification and delivers the end-to-end DuckDB queryability that was the headline goal of v0.20.0–v0.22.0.

**Catalog schema upgrade — 28-table DuckLake v1.0 spec**
- **Rewrite `DuckLakeSink::ensure_catalog()`** — replace the 8-table schema with the full 28-table DuckLake v1.0 catalog: `ducklake_snapshot`, `ducklake_snapshot_changes`, `ducklake_schema`, `ducklake_table`, `ducklake_column`, `ducklake_data_file`, `ducklake_delete_file`, `ducklake_file_column_stats`, `ducklake_table_stats`, `ducklake_table_column_stats`, `ducklake_metadata`, `ducklake_inlined_data_tables`, `ducklake_partition_info`, `ducklake_partition_column`, `ducklake_tag`, `ducklake_view`, `ducklake_macro`, `ducklake_secret`, `ducklake_cached_secret`, `ducklake_database_configuration`, and the remaining supporting tables per the v1.0 spec. For the PostgreSQL-backed path, retain `CREATE SEQUENCE` for `table_id`, `schema_id`, `column_id`, and `file_id` (these are catalog-entity sequences — they remain valid for the PG path). Remove the `ducklake_snapshot_id_seq` sequence entirely; snapshot IDs are now derived from `next_catalog_id` in the previous snapshot row.
- **Remove `table_id` from `ducklake_snapshot`** — the v1.0 spec defines snapshots as catalog-wide, not per-table. The current `ducklake_snapshot(snapshot_id, table_id, schema_version, sequence_number, author)` schema is replaced with the spec-compliant `ducklake_snapshot(snapshot_id, snapshot_time, schema_version, next_catalog_id, next_file_id, author)` shape. The `table_id` column is removed; per-table change tracking moves entirely into `ducklake_snapshot_changes`.
- **Remove `table_id` from `ducklake_snapshot_changes`** — align with the v1.0 spec: changes reference the catalog snapshot ID, not a specific table. The per-table association is already expressed by the `data_file_id` and `schema_id` foreign keys in the change rows.

**ID allocation — snapshot-carried counters**
- **Remove `ducklake_snapshot_id_seq`** — the next snapshot ID is now `max(snapshot_id) + 1` read from `ducklake_snapshot` at startup and maintained in-process after each commit. This matches the RockLake path and the real DuckLake spec; no `nextval()` is needed for snapshot IDs.
- **`refresh_id_state()` helper** — on sink startup and after any catalog reconnect, read `(next_catalog_id, next_file_id, schema_version)` from the most recent `ducklake_snapshot` row. In-process counters track the next available ID within the current session. `next_catalog_id` covers schema, table, column, and partition entity IDs; `next_file_id` covers data-file and delete-file IDs. Retain PostgreSQL sequences for `table_id`, `schema_id`, `column_id`, and `file_id` as the canonical backing store; `next_catalog_id` and `next_file_id` in the snapshot record the high-water mark committed so far.
- **`DuckLakeSink::publish()` rewrite** — pre-allocate IDs before beginning the catalog transaction; build all INSERT statements with literal ID values (no `RETURNING`-based ID capture for the main IDs). The existing `RETURNING table_id` in `bootstrap_table()` is retained (it is valid PostgreSQL and produces no spec incompatibility), but the snapshot-level counters no longer depend on it.

**`ducklake_common` module completion**
- **Promote to authoritative shared module** — the `ducklake_common` module extracted in v0.37.0 as an RockLake-specific scaffold is promoted to the single implementation backing both `DuckLakeSink` (PostgreSQL path) and `RockLakeSink` (RockLake sidecar path). It now owns: `parquet_build()`, `column_stats_compute()`, `schema_evolution_detect()`, `DuckLakePartition`, `SchemaChangePolicy`, `catalog_ids::IdState`, and `inlined_data::InlinedWriter`.
- **`IdState` struct** — encapsulates `(last_snapshot_id, next_catalog_id, next_file_id, schema_version)` with typed `alloc_catalog_id()` and `alloc_file_id()` increment methods. Both sinks load `IdState` from the latest snapshot row at startup and pass it through publish calls. This eliminates the previous divergence where `DuckLakeSink` used `nextval()` and `RockLakeSink` used manual counters.

**Data inlining alignment**
- **Update `ducklake_inlined_data_tables` registry** — register each new inlined-data table in `ducklake_inlined_data_tables(table_id, schema_version)` as required by the v1.0 spec. The `ensure_inlined_table()` helper now issues a SELECT existence check before `CREATE TABLE IF NOT EXISTS`, consistent with the RockLake path.
- **Remove `ON CONFLICT DO NOTHING` from inlined data registration** — replace with an explicit existence SELECT + conditional INSERT, making the PG path consistent with the RockLake path and future-proofing for any DuckLake client that flags `ON CONFLICT` as non-standard.

**Schema evolution alignment**
- **`ducklake_column.begin_snapshot` and `end_snapshot`** — additive schema evolution (new JSON keys in message payloads) now correctly sets `begin_snapshot = new_snapshot_id` and leaves `end_snapshot = NULL` on the new `ducklake_column` row. Removed columns (if the `on_schema_change = pause` policy is not applied) set `end_snapshot = new_snapshot_id` on the old column row, enabling DuckLake MVCC projection over historical Parquet files.
- **`schema_version` counter** — `DuckLakeSink` now increments `schema_version` in the snapshot row whenever the set of live columns for a table changes, consistent with the DuckLake spec's expectation that schema_version tracks the inlining layout epoch.

**Migration path for existing deployments**
- **`pg_tide--0.38.0--0.39.0.sql` migration script** — adds the 18 missing catalog tables (`ducklake_delete_file`, `ducklake_partition_info`, `ducklake_partition_column`, `ducklake_tag`, `ducklake_view`, `ducklake_macro`, `ducklake_secret`, `ducklake_cached_secret`, `ducklake_database_configuration`, `ducklake_inlined_data_tables`, and the remaining supporting tables), adds the `next_catalog_id` and `next_file_id` columns to `ducklake_snapshot`, drops the `table_id` column from `ducklake_snapshot` and `ducklake_snapshot_changes`, and drops the `ducklake_snapshot_id_seq` sequence. Wrapped in a transaction with a `BEGIN`/`COMMIT` and a `RAISE NOTICE` banner.
- **`tide.ducklake_migrate_catalog(schema_name TEXT DEFAULT 'ducklake')` SQL function** — a one-time idempotent migration helper that: (1) back-fills `next_catalog_id` and `next_file_id` in every existing `ducklake_snapshot` row by computing the high-water mark of all catalog-entity IDs at each snapshot boundary; (2) marks migrated rows with an `author = 'pg-tide-migrate'` annotation. Safe to call on a live database with the relay stopped; returns a summary row `(snapshots_migrated INT, data_files_verified INT)`. No data loss — all Parquet file paths and column stats are preserved.
- **Operator checklist** — document in the v0.38.0→v0.39.0 section of the upgrade guide: (1) stop relay; (2) `ALTER EXTENSION pg_tide UPDATE`; (3) `SELECT tide.ducklake_migrate_catalog()`; (4) restart relay. The relay startup guard in `DuckLakeSink::ensure_catalog()` verifies `next_catalog_id IS NOT NULL` in the latest snapshot and refuses to publish until migration is complete, emitting a clear error message pointing to the migration helper.

**End-to-end DuckDB integration tests**
- **`tests/ducklake_duckdb_test.rs`** — new integration test module that: (1) starts a PostgreSQL 18 testcontainer with `pg_tide` installed; (2) configures a `DuckLakeSink` writing to a local directory via `object_store::local::LocalFileSystem`; (3) publishes 200 synthetic events across two tables (`order_created` and `payment_processed`) with 3 schema-evolution cycles; (4) connects DuckDB to the PostgreSQL catalog via `ATTACH 'ducklake:postgres://...'`; (5) asserts all 200 rows are queryable; (6) issues `AT (VERSION => N)` time-travel queries and asserts correct row counts per snapshot; (7) runs `EXPLAIN SELECT * FROM orders WHERE order_id = 42` and verifies DuckDB emits a Parquet file-pruning annotation in the plan (confirming column statistics are used for filter pushdown); (8) adds a new column to `payment_processed`, publishes 50 more events, and asserts the schema evolution is reflected in `ducklake_column` and readable from DuckDB.
- **`atomic_lake_writes` correctness test** — with `atomic_lake_writes = true`, kill the relay process immediately after the Parquet file is written but before the catalog `COMMIT`. Restart and assert: zero duplicate rows in DuckDB; the orphaned Parquet file is not referenced in any `ducklake_data_file` row; the relay consumer offset has not advanced beyond the pre-crash value.
- **Schema evolution round-trip test** — publish 50 events with schema A, then 50 with schema B (one new column added), then query both ranges from DuckDB and assert older rows return `NULL` for the new column (DuckLake nullable column projection).
- **Column statistics filter-pushdown test** — publish 1 000 events with `order_total` values between 0 and 1 000. Query `WHERE order_total BETWEEN 800 AND 1000` and assert via `pg_stat_statements` that only the relevant Parquet files were accessed (row count in `pg_stat_statements` is < 200), confirming `ducklake_file_column_stats` min/max ranges are honoured by DuckDB's query planner.

**`DuckLakeSource` data-plane completion**
- **Snapshot polling on catalog-wide snapshots** — update `DuckLakeSource::poll()` to use the spec-correct `SELECT max(snapshot_id) FROM ducklake_snapshot WHERE snapshot_id > $1` (no `table_id` predicate). The previous query joined on `table_id` which was both non-spec and incompatible with the new schema.
- **Data file fetch** — implement the full incremental data fetch: `SELECT data_file_id, path, record_count, begin_snapshot, file_size_bytes, file_order FROM ducklake_data_file WHERE table_id = $1 AND begin_snapshot > $2 AND begin_snapshot <= $3 AND (end_snapshot IS NULL OR end_snapshot > $3) ORDER BY file_order ASC`. Reads Parquet bytes from object storage, decodes each row using the stored column schema, and emits `RelayMessage` values. Deduplication via `_dedup_key` column (stored in Parquet by the sink).
- **Delete file handling** — when a `ducklake_delete_file` row exists for a data file, apply positional deletions before emitting messages. Adds tombstone support for DuckLake → pg-tide inbox reverse pipelines that reflect row-level deletes.
- **Integration test** — configure a round-trip: (1) `DuckLakeSink` writes 100 events; (2) DuckDB directly inserts 20 enriched rows into the same catalog; (3) `DuckLakeSource` delivers all 120 rows to a pg-tide inbox; (4) assert zero duplicates after replaying the same snapshot range twice.

**`atomic_lake_writes` same-transaction mode hardening**
- **Re-validate with spec-compliant catalog writes** — the `atomic_lake_writes = true` mode (introduced in v0.20.0) commits both the outbox consumer-offset advance and the DuckLake catalog snapshot in a single PostgreSQL transaction. With the v1.0 spec catalog, the transaction boundary is unchanged but the write set is larger (28-table schema vs. 8-table). Re-run the crashloop correctness tests (introduced in v0.23.0) against the new schema to confirm exactly-once semantics hold.
- **`pg-tide doctor` DuckLake spec version check** — extend the doctor command to query `SELECT value FROM ducklake_metadata WHERE key = 'ducklake_spec_version'` (written by `ensure_catalog()` on first init) and emit a `WARNING` if the value is missing or less than `"1.0"`, prompting the operator to run `tide.ducklake_migrate_catalog()` before starting the relay.
