...
| `PGTIDE_EXTENSION_VERSION_INCOMPATIBLE` | cli | The installed pg_tide extension is outside the relay lifecycle policy. | false | 1 | `upgrade-failed` |
...
| `PGTIDE_CATALOG_MISSING` | cli | The pg_tide catalog is missing. | false | 1 | `upgrade-failed` |
| `PGTIDE_PIPELINE_NOT_FOUND` | cli | The requested pipeline was not found. | false | 1 | `pipeline-not-discovered` |
| `PGTIDE_PIPELINE_INVALID` | cli | The pipeline configuration is invalid. | false | 1 | `relay-will-not-start` |
| `PGTIDE_PIPELINE_NOT_DISCOVERED` | cli | The pipeline was not discovered by the relay. | false | 1 | `pipeline-not-discovered` |
| `PGTIDE_PIPELINE_NOT_OWNED` | cli | The relay does not own the requested pipeline. | true | 1 | `advisory-lock-ownership` |
...
| `PGTIDE_CONNECTOR_AUTHENTICATION` | cli | The destination connector rejected authentication. | false | 1 | `sink-authentication-failure` |
| `PGTIDE_CONNECTOR_AUTHORIZATION` | cli | The destination connector denied authorization. | false | 1 | `sink-authentication-failure` |
| `PGTIDE_TLS_VERIFICATION_FAILED` | cli | TLS verification failed. | false | 1 | `relay-will-not-start` |
| `PGTIDE_WEBHOOK_SSRF_REJECTED` | cli | The webhook destination was rejected by SSRF protection. | false | 1 | `webhook-connector` |
...
| `PGTIDE_REPLAY_INPUT_INVALID` | cli | The replay or DLQ input is invalid. | false | 1 | `dlq-is-growing` |
| `PGTIDE_MAINTENANCE_SWEEP_FAILED` | cli | The maintenance sweep could not be completed. | true | 1 | `retention-cleanup` |
| `PGTIDE_SHUTDOWN_FAILED` | cli | The relay could not complete graceful shutdown. | true | 1 | `advisory-lock-ownership` |
...
| `PGTIDE_CONFIG_UNSUPPORTED_SURFACE` | sql | The requested configuration surface is not supported. | false | — | `upgrade-failed` |
...
| `PGTIDE_RELAY_NOT_FOUND` | sql | The relay pipeline was not found. | false | — | `pipeline-not-discovered` |
| `PGTIDE_INVALID_ARGUMENT` | sql | A SQL function argument is invalid. | false | — | `relay-will-not-start` |
| `PGTIDE_SWEEP_FAILED` | sql | The outbox sweep failed. | true | — | `retention-cleanup` |
