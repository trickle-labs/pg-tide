...
///
/// Each entry is inserted individually.  Insertion errors are logged but do
/// not propagate — we must not lose the DLQ record because of a DB transient.
pub async fn insert_batch(db: &Arc<Client>, entries: &[DlqEntry]) -> Result<(), RelayError> {
    for entry in entries {
        insert_one(db, entry).await?;
    }
    Ok(())
}
