...
def render_descriptors(rows: list[dict]) -> str:
    supported_rows = [
        ...
        and row["maturity"] == "supported"
        and row.get("capabilities")
    ]
    supported_source_types = sorted(
        {
            runtime_type
            for row in supported_rows
            for runtime_type in path_list(row, "source_types")
        }
    )
    supported_sink_types = sorted(
        {
            runtime_type
            for row in supported_rows
    ...
    lines.extend(
        [
            ...
            "",
            f"pub const V1_SUPPORTED_SOURCE_TYPES: &[&str] = {rust_string_list(supported_source_types)};",
            f"pub const V1_SUPPORTED_SINK_TYPES: &[&str] = {rust_string_list(supported_sink_types)};",
            "",
